...
Alpha LMS follows a **structured, machine-readable, context-rich** logging approach. Every log record is a JSON object (in production) or coloured line (in development) carrying enough context to reconstruct what happened, who triggered it, and what the system state was — without querying the database.

Two complementary layers:
1. **Application logs** — `structlog` JSON to stdout; shipped to Loki / CloudWatch Logs.
2. **Compliance audit trail** — INSERT-only rows in `audit_logs` table; 7-year retention (RBI IT Framework 2017).

These are not substitutes. Application logs are operational (ops, engineering); `audit_logs` is the regulatory record.

### 18.2 Log Levels

| Level | Used For |
|---|---|
| `DEBUG` | Detailed internals — query plans, allocation bucket traces. Dev / local only; never enabled in prod. |
| `INFO` | Normal lifecycle events — loan created, payment posted, cron completed. Default production level. |
| `WARNING` | Degraded-but-recoverable state — NPA classification, bounce recorded, no mandate after NPA upgrade. |
| `ERROR` | Unhandled exceptions, dependency failures (DB timeout, external API 5xx). Always alerted. |
| `CRITICAL` | Service unable to start; security violation detected. PagerDuty P0. |

### 18.3 Mandatory Context Fields

Every log record (regardless of level) carries:

| Field | Source |
|---|---|
| `timestamp` | Structlog `TimeStamper(fmt="iso")` — always UTC |
| `level` | Log level string |
| `logger` | Python module path (`src.services.payment_service`) |
| `service` | Hard-coded `alpha-lms` |
| `request_id` | `X-Request-Id` header — read from `request_id_ctx` ContextVar. `null` for cron jobs. |

### 18.4 HTTP Request Logging

Handled by `access_log_middleware` in `main.py`. Records `method`, `path`, `status` at `INFO`. Raw `uvicorn.access` is suppressed to avoid duplication. Stack traces from unhandled exceptions are logged at `ERROR` with `exc_info=True` via `generic_error_handler`.

### 18.5 Sensitive Data Redaction

The `_mask_event` structlog processor runs on every log record before emission:
- PAN numbers (`XXXXXNNNNX` pattern) → `***PAN***`
- Aadhaar numbers (12-digit with optional spaces/hyphens) → `***AADHAAR***`

This is enforced at the logging layer — not a convention. Code that accidentally logs a PAN will have it stripped automatically.

**What must NEVER appear in logs:**
- Plaintext PAN / Aadhaar (masked automatically)
- Bank account / IFSC numbers (ops practice — no automated masking yet)
- Passwords, API keys, webhook secrets (use environment variables; never pass to logger)

### 18.6 Development vs Production

| Concern | Development (`LOG_JSON=false`) | Production (`LOG_JSON=true`) |
|---|---|---|
| Format | Coloured, human-readable (structlog `ConsoleRenderer`) | Newline-delimited JSON |
| Level | `DEBUG` | `INFO` |
| Destination | stdout (terminal) | stdout → Loki / CloudWatch |
| `request_id` | Present if header sent | Always present (generated by middleware) |
| SQLAlchemy engine | `WARNING` | `WARNING` (never echo SQL in prod) |

### 18.7 Cron Job Logging Pattern

Every cron function logs a single `INFO` record at completion with a result summary:
```json
{ "event": "dpd_cron_complete", "processed": 412, "date": "2026-05-28", "service": "alpha-lms" }
```
Cron records have no `request_id` (null). The `date` field enables Loki range queries for operational dashboards.

### 18.8 Compliance Audit Events

The following state changes write to both application logs AND `audit_logs`:

| Event | App Log Level | `audit_logs` action |
|---|---|---|
| Loan created | `INFO` | `loan_created` |
| Payment posted | `INFO` | `payment_posted` |
| Bounce recorded | `WARNING` | `bounce_recorded` |
| Loan foreclosed | `INFO` | `loan_foreclosed` |
| NPA classified | `WARNING` | `npa_classified` |
| NPA upgraded | `INFO` | `npa_upgraded` |
| Write-off initiated | `INFO` | `write_off_initiated` |
| Write-off approved | `INFO` | `write_off_approved` |
| Recovery posted | `INFO` | `recovery_posted` |

See LLD §15 for the full field-level specification.
